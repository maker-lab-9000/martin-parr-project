...
**Interfaces:**
- Produces: `Picamera2Camera(tuning_file=..., save_dng=True, autofocus="continuous", af_range="normal", ae_lock=False, awb_lock=False, colour_gains: tuple[float, float] | None = None)`. CLI: `--autofocus {continuous,auto,manual}` (default continuous), `--af-range {normal,macro,full}` (default normal), `--ae-lock`, `--awb-lock`, `--colour-gains R,B`, `--no-dng`. All Picamera2-only; argparse errors if combined with `--camera v4l2`.
