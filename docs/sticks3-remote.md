...
The examples use account `george`, hostname `parr`, project checkout
`/home/george/repos/martin-parr-project`, hotspot `parr-cam` at `10.42.0.1`.
...
- `PARR_REMOTE_URL` is the address the Stick calls. Its host must be a plain
...
```sh
cd /home/george/repos/martin-parr-project
...
The script writes `/etc/NetworkManager/system-connections/parr-ap.nmconnection`
with mode 0600, reloads NetworkManager and activates `parr-ap`. The profile is
...
From a phone or laptop joined to `parr-cam` (or from the Pi itself):
...
```sh
.venv/bin/parr-capture --no-preview --show-captures --remote-listen 0.0.0.0:8765 --artifacts <verified-pi-artifact-directory>
```
...
terminal controls. If `journalctl -u parr-capture.service` shows
`error: --remote-listen requires PARR_REMOTE_TOKEN` in a restart loop, the
environment file is not delivering the variable. Check its shape without
printing the secret, and confirm the key is spelled exactly `PARR_REMOTE_TOKEN`
...
while retaining the base environment build flags. `PARR_REMOTE_URL` becomes the
...
`PARR_REMOTE_URL` set. On a clean clone with no `.env`, it adds no defines and
...
Then confirm the device joins `parr-cam`, reaches the Pi, and produces one
...
`sudo nmcli connection down parr-ap` followed by
`sudo nmcli connection up <home-wifi-profile>`. Re-enable the hotspot with
`sudo nmcli connection up parr-ap`.
