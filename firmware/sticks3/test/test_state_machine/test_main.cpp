...
void settle_button(CaptureClient& client, uint32_t at_ms, const char* id) {
  ...
  TEST_ASSERT_TRUE(client.onButtonSample(true, at_ms + 30, id));
}
...
void test_restart_or_missing_job_does_not_create_a_replacement_request(void) {
  CaptureClient client;
  client.setWifiConnected(true, 0);
  settle_button(client, 10, "11111111-1111-4111-8111-111111111111");
  ...
  TEST_ASSERT_EQUAL_STRING("11111111-1111-4111-8111-111111111111", client.requestId());
  TEST_ASSERT_EQUAL(WorkKind::Poll, client.nextWork(1050).kind);
}
...
void test_wifi_reconnect_backoff_starts_at_one_second_and_doubles(void) {
  CaptureClient client;
...
void setup() {
  ...
  RUN_TEST(test_restart_or_missing_job_does_not_create_a_replacement_request);
  ...
  RUN_TEST(test_wifi_reconnect_backoff_starts_at_one_second_and_doubles);
...
int main() {
  ...
  test_restart_or_missing_job_does_not_create_a_replacement_request();
  ...
  test_wifi_reconnect_backoff_starts_at_one_second_and_doubles();
